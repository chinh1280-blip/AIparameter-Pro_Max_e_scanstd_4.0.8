...
import { X, Save, Link, Plus, Trash2, Check, Layers, RefreshCw, Key, BrainCircuit, Edit3, Trash, Settings2, Box, Search, Copy, Tag, Database, Cloud, Cpu, Monitor, Scan, Lock, Palette, Sun, Moon, Type, Info } from 'lucide-react';
import { StandardDataMap, ProductPreset, ModelConfig, Machine, ZoneDefinition, ScanConfig, UIConfig, DEFAULT_UI_CONFIG } from '../types';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({
    ...
    try {
      await fetch(googleSheetUrl, {
        ...
        body: JSON.stringify({
          ...
          id: isEditing ? editingPresetId : undefined, // Pass ID if editing to update instead of insert
  ...
  const handleUIConfigChange = (key: keyof UIConfig, value: string) => {
    if (setUiConfig) {
            ...
            <div className="absolute inset-0 bg-slate-900 z-50 flex flex-col p-6 animate-fade-in overflow-y-auto">
                ...
                <div className="grid grid-cols-1 gap-3">
                    {scanOptions.map((opt, idx) => (
                        ...
                        >
                            <div>
                                <p className="text-xs font-black text-white uppercase mb-1">Khổ: {opt.filmWidth || "Không xác định"}</p>
