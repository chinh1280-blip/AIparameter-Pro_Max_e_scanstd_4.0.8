        ...
        <div className="p-4 pt-0 bg-slate-950/40 border-t border-slate-800/50 animate-slide-down">
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-2 mt-4">
            {logDataKeys.map(f => {
              ...
              // Border color logic based on tolerance
              let borderColor = 'border-slate-800'; // Default
              if (std > 0) { // Only color if there is a standard
                  if (diffAbs <= tol / 2) {
                      borderColor = 'border-green-500/50 shadow-[0_0_10px_-2px_rgba(34,197,94,0.3)]'; // Green border (OK)
                  } else if (diffAbs <= tol) {
                      borderColor = 'border-yellow-500/50 shadow-[0_0_10px_-2px_rgba(234,179,8,0.3)]'; // Yellow border (Warning)
                  } else {
                      borderColor = 'border-red-500/50 shadow-[0_0_10px_-2px_rgba(239,68,68,0.3)]'; // Red border (Fail)
                  }
              }
